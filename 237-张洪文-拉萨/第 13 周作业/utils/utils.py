...
class BBoxUtility(object):
    ...
    def detection_out(self, predictions, mbox_priorbox, num_classes, keep_top_k=300,confidence_threshold=0.5):
        """
        通过非极大抑制算法来去除多余的预测框
        :param predictions: 网络预测的结果，包括置信度（mbox_conf）和位置（mbox_loc）。
        :param mbox_priorbox: 先验框的信息。
        :param num_classes: 目标检测的类别数量。
        :param keep_top_k: 保留置信度最高的前k个预测结果，默认为300。
        :param confidence_threshold: 置信度阈值，只有置信度大于该值的预测结果才会被保留，默认为0.5。
        :return:
        """
        # 网络预测的结果: 置信度、位置、先验框信息
